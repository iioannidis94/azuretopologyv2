...
export function generatePowerShell(){
  ...
  const deploymentReady = validation.errors === 0;
  if (deploymentReady) {
    lines.push(`# ✓ DEPLOYMENT STATUS: READY`);
    lines.push(`# This script has passed validation and is ready for deployment.`);
    lines.push(`#`);
  } else {
    lines.push(`# ✗ DEPLOYMENT STATUS: NOT READY`);
    lines.push(`# This script requires manual fixes before deployment.`);
    lines.push(`# ${validation.errors} resource(s) have missing required fields.`);
    lines.push(`#`);
  }
  ...
  if (validation.errors > 0) {
    lines.push(`# ⚠️ BEFORE DEPLOYMENT - REQUIRED ACTIONS:`);
    lines.push(`# 1. Search for all <REQUIRED:...> placeholders in this script`);
    lines.push(`# 2. Replace each placeholder with the actual value`);
    lines.push(`# 3. Run the validation tool to verify 100% deployability:`);
    lines.push(`#    .\\Validate-AzureDeployment.ps1 -ScriptPath ".\\azure-deploy.ps1" -CheckAzModule -DetailedReport`);
    lines.push(`# 4. Once validation passes, proceed with deployment`);
    lines.push('');
  } else {
    lines.push(`# ✓ DEPLOYMENT INSTRUCTIONS:`);
    lines.push(`# 1. Ensure you have Azure PowerShell installed: Install-Module -Name Az -Force`);
    lines.push(`# 2. Authenticate to Azure: Connect-AzAccount`);
    lines.push(`# 3. Set the correct subscription: Set-AzContext -Subscription "<your-subscription>"`);
    lines.push(`# 4. (Optional) Validate with: .\\Validate-AzureDeployment.ps1 -ScriptPath ".\\azure-deploy.ps1"`);
    lines.push(`# 5. Run this script: .\\azure-deploy.ps1`);
    lines.push(`#`);
    lines.push(`# For detailed deployment instructions, see: DEPLOYMENT-GUIDE.md`);
    lines.push('');
  }
