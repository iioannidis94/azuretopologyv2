export const computeCategory = {
  ...
  resources: {
    vm: {
      ...
      fieldOptions: {
        os: { options: ['Ubuntu 22.04', 'Ubuntu 24.04', 'Windows Server 2022'] },
        osDiskType: { options: ['Premium_LRS', 'StandardSSD_LRS', 'Standard_LRS'] },
        dataDiskType: { options: ['Premium_LRS', 'StandardSSD_LRS', 'Standard_LRS'] },
        authType: { options: ['SSH Key', 'Password'] },
        availabilityZone: { options: ['None', '1', '2', '3'] },
        managedIdentity: { options: ['SystemAssigned', 'UserAssigned', 'None'] },
        patchMode: { options: ['AutomaticByPlatform', 'ImageDefault', 'Manual'] },
        securityType: { options: ['TrustedLaunch', 'Standard'] }
      },
    ...
    vmss: {
      ...
      fieldOptions: {
        upgradePolicy: { options: ['Rolling', 'Automatic', 'Manual'] },
        os: { options: ['Ubuntu 22.04', 'Ubuntu 24.04', 'Windows Server 2022'] },
        osDiskType: { options: ['Premium_LRS', 'StandardSSD_LRS', 'Standard_LRS'] }
      },
    ...
    aks: {
      ...
      fieldOptions: {
        networkPlugin: { options: ['azure', 'kubenet'] },
        tier: { options: ['Standard', 'Free'] }
      },
    ...
    fa: {
      ...
      fieldOptions: {
        plan: { options: ['Consumption', 'ElasticPremium', 'Premium', 'Dedicated'] },
        runtime: { options: ['node', 'dotnet', 'python', 'java', 'powershell'] },
        runtimeVersion: { options: ['8', '18', '20', '21', '22'] },
        osType: { options: ['Linux', 'Windows'] }
      },
    ...
    aca: {
      ...
      fieldOptions: {
        ingress: { options: ['external', 'internal'] },
        cpu: { options: ['0.25', '0.5', '1', '2'] },
        memory: { options: ['0.5Gi', '1.0Gi', '2.0Gi', '4.0Gi'] }
      },
